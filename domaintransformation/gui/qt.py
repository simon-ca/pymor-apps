...
from domaintransformation.gui.gl import GLPatchWidget, ColorBarWidget, HAVE_GL
...
def visualize_patch(grid, U, mu, bounding_box=([0, 0], [1, 1]), codim=2, title=None, legend=None,
    ...
    class MainWindow(PlotMainWindow):
        def __init__(self, grid, U, mu, bounding_box, codim, title, legend, separate_colorbars, rescale_colorbars, backend):
            ...
            assert isinstance(mu, Parameter) \
                or isinstance(mu, tuple) and all(isinstance(m, Parameter) for m in mu) and len(mu) == len(U)
            ...
            class PlotWidget(QWidget):
                def __init__(self):
                    ...
                    plots = [widget(self, grid, m, vmin=vmin, vmax=vmax, bounding_box=bounding_box, codim=codim)
                             for vmin, vmax, m in izip(self.vmins, self.vmaxs, mu)]
                    if legend:
    ...
    _launch_qt_app(lambda: MainWindow(grid, U, mu, bounding_box, codim, title=title, legend=legend,
                                      separate_colorbars=separate_colorbars, rescale_colorbars=rescale_colorbars,
                                      backend=backend),
                   block)


class DomainTransformationPatchVisualizer(BasicInterface):
    """Visualize scalar data associated to a two-dimensional |Grid| as a patch plot.
    ...
    def visualize(self, U, discretization, mu=None, title=None, legend=None, separate_colorbars=False,
        ...
        if filename:
            ...
            visualize_patch(self.grid, U, mu, bounding_box=bounding_box, codim=self.codim, title=title,
