...
def flatten_grid(grid, mu):
    ...
    assert isinstance(grid, DomainTransformationTriaGrid)
    assert isinstance(mu, Parameter)

    if not grid.identify_left_right and not grid.identify_bottom_top:
        subentities = grid.subentities(0, grid.dim)
        coordinates = grid.centers(grid.dim, mu)
